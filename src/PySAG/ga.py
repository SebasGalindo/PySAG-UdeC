"""Clase principal en donde se implementa el algoritmo genético."""
...
class GA:
    ...
    def __init__(
        ...
        uses_mutation_rate_arg = (
            self.mutation_func == default_mutation.mutation_random_gene_uniform
            or self.mutation_func == default_mutation.mutation_bit_flip
            or self.mutation_func == default_mutation.mutation_swap
        )
        if "mutation_rate" not in self.mutation_args and uses_mutation_rate_arg:
            # Solo para los que usan mutation_rate como arg directo
    ...
    def _calculate_population_fitness(self):
        """Calcula la aptitud de todos los individuos en la población."""
    ...
    def run(self):
        ...
        for generation in range(self.num_generations):
            ...
            print(
                ...
                f"Mejor Fitness = {current_best_fitness:.4f}"
            )

            elite_individuals = []
            if (
                self.keep_elitism_count > 0 and self.population
            ):  # Añadir check para población no vacía
            ...
            if not parents or (len(parents) < 2 and self.crossover_func is not None):
                ...
                while len(offspring_population) < num_offspring_to_generate:
                    ...
                    if not source_pop_for_cloning:
                        break

                    cloned_parent = source_pop_for_cloning[
                        parent_idx % len(source_pop_for_cloning)
                    ].copy()
                ...
                while len(offspring_population) < num_offspring_to_generate:
                    ...
                    parent_idx = (
                        parent_idx + 2
                    )  # Avanza sin % aquí, se controla con el len(parents) en el acceso
                    if parent_idx >= len(
                        parents
                    ):  # Ciclar manualmente los padres si se acaban
                        parent_idx = 0

            mutated_offspring_population = []
            for individual in offspring_population:
                mutated_individual = self.mutation_func(
                    individual, **self.mutation_args
                )
                mutated_offspring_population.append(mutated_individual)

            self.population = (
                elite_individuals
                + mutated_offspring_population[:num_offspring_to_generate]
            )
    ...
    def plot_fitness(self):
        ...
        try:
            ...
            plt.plot(
                self.best_solutions_fitness, marker="o", linestyle="-", markersize=4
            )
            ...
            print(
                ...
                "Instala con 'pip install pysag[plot]' o 'pip install matplotlib'"
            )
